...
* **Transparencia de Modelo y Razonamiento Adaptativo (Future-Proof):**
  * **Descubrimiento Dinámico de Capacidades:** No depende de listas fijas de modelos ni de nombres hardcodeados; interroga a la CLI (`agy models`) en tiempo de ejecución para descubrir el abanico de niveles de razonamiento del modelo activo. Si el modelo no admite ajuste de razonamiento (ej. variantes únicas o modelos sin parámetros de esfuerzo), opera de forma transparente sin flags innecesarios.
  * **Razonamiento Adaptativo por Tarea (`DYNAMIC_REASONING_EFFORT = True`):** Utiliza automáticamente el nivel mínimo de razonamiento para consultas cotidianas (minimizando latencia a menos de 3s), y escala de forma dinámica al nivel máximo cuando se detecta una tarea especial de desarrollo o modificación sobre el código interno del asistente.
  * **Invarianza de Modelo:** El modelo base elegido por el usuario permanece constante a lo largo de toda la conversación; solo conmuta el esfuerzo de razonamiento en la misma sesión interactiva (`-c`).
  * **Identificación del Nivel de Razonamiento en Cada Respuesta:** Presenta en el pie (*subtitle*) del panel de respuesta en consola el nivel exacto de razonamiento aplicado (`⚡ Razonamiento: Alto (High)`, `⚡ Razonamiento: Bajo (Low)` o `⚡ Razonamiento: Estándar`), reflejando con total certeza el nivel ejecutado por la CLI sin sobrecargar la pantalla con telemetría accesoria.
  * **Tolerancia y Fallback Defensivo:** Si una versión futura de la CLI o un modelo específico rechaza el argumento de esfuerzo, el lanzador reintenta inmediatamente la instrucción sin dicho flag sin interrumpir la experiencia del usuario.
...
| `DYNAMIC_REASONING_EFFORT` | `True` | Habilita conmutación adaptativa (mínimo para tareas cotidianas, máximo para código interno). |
